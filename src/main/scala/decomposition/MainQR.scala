...
object MainQR {
  def main(args: Array[String]) {
    ...
    //val pathToData = "hdfs://" + master + ":9000/matrix.txt"

    val data = sc.textFile(pathToData)
    val matrix = new RowMatrix(data.map(s => Vectors.dense(s.split(' ').map(_.toDouble))))

    val trueR = matrix.tallSkinnyQR(false).R.rowIter.foreach(println)
    //val trueR = matrix.tallSkinnyQR(false).R.toArray.foreach(println)
    ...
    val reducedMatrix = blockMatrix.map(block => blockProcess(block, miniBatchSize, sc))
